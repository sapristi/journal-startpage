...
export const makeSyncEntriesStore = (name, initData) => {
  ...
  const checkKey = (key) => {
    if (!key.startsWith(prefix)) {
      throw new Error(`Key should start with ${prefix}`);
    }
  };
  ...
  const getEntries = (callback) =>
    storage.get(null, (entries) => {
      return callback(
        filterObject(entries, ([key, value]) => key.startsWith(prefix)),
      );
    });
  const getEntriesAsync = () => {
    return new Promise((resolve, reject) => {
      storage.get(null, (entries) => {
        const result = filterObject(entries, ([key, value]) =>
    ...
    switch (action.type) {
      ...
      case "updateOnChange":
        ...
        return {
          ...filterObject(
            state,
            ([key, value]) => !removedEntries.includes(key),
          ),
